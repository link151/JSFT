...
.controller('acceptCtrl',['$scope','$rootScope','$http','$location','$cookies','$cookieStore',
	...
	if($location.absUrl().indexOf('addAcceptCredit') && $rootScope.role!='A'){
		$location.path('/searchAcceptCredit');
	}
...
//		$('#closeModal').click();
	...
	$scope.exportExcel = function(){
		var contractid = $('.bootstrap-table-filter-control-contractid').val();
		var bank = $('.bootstrap-table-filter-control-bank').val();
		var currency = $('.bootstrap-table-filter-control-currency').val();
		var paydate = $('.bootstrap-table-filter-control-paydate').val();
		var registerdate = $('.bootstrap-table-filter-control-registerdate').val();
		var payamount = $('.bootstrap-table-filter-control-payamount').val();
		var obj = {'registerdate' : registerdate, 'contractid' : contractid, 'bank' : bank, 'currency' : currency, 
				'paydate' : paydate, 'payamount' : payamount};
		
		$http.post('AcceptExportExcel.do', obj, {responseType: 'arraybuffer'}).success(function(data){
	...
	$scope.deleteAccept = function(){
		...
		if(length > 0){
			...
			if(want_delete == true){
				...
				$http.post('AcceptDelete.do', ids).success(function(success){
					alert('删除成功');
					$('#table').bootstrapTable('refresh');
				}).error(function(error){
					alert('删除失败');
